...
final class JdbcIndexerSpec
  ...
  private val noOpFlow = Flow[OffsetStepUpdatePair[Update]].map(_ => ())
  ...
  it should "process updates with correct offset steps on subscription" in {
    val participantId = "the-participant"
    val sentinel = mutable.ArrayBuffer.empty[OffsetStepUpdatePair[Update]]
    val flow =
    ...
    initializeIndexer(participantId, flow)
      ...
      .map(_ => sentinel should contain theSameElementsInOrderAs expected)
  }
  ...
  private def initializeIndexer(
      ...
      mockFlow: Flow[OffsetStepUpdatePair[Update], Unit, NotUsed] = noOpFlow,
  ...
  private def mockedUpdateFlowOwnerBuilder(
    ...
    when(
      ...
      .thenReturn(ResourceOwner.successful(mockFlow))
    mocked
  }
}
